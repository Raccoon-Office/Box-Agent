...
*   ✅ **Jupyter Sandbox Execution**: Isolated `execute_code` tool runs Python in a sandboxed venv with its own kernel — safe from host environment pollution. In standalone runtime mode, uses an in-process kernel with bundled packages — no external Python required.
...
*   ✅ **Auto Package Install**: Detects `ModuleNotFoundError` at runtime and automatically `pip install`s missing packages (sklearn → scikit-learn, cv2 → opencv-python, PIL → Pillow, etc.) with retry. In runtime mode, installs to `~/.box-agent/runtime-packages/` with whitelist control.
...
*   ✅ **Standalone Runtime**: PyInstaller-packaged binary bundles Python + all dependencies including data science packages (pandas, numpy, matplotlib, scikit-learn) and pip for on-demand installs. No external Python required — just download and run. Available for macOS (arm64/x64) and Linux.
