...
durable Surface blocks, and provider-only request projections.
"""
...
def adapt_mcp_durable_inline_images(
    *, server_name: str, remote_name: str, inline_images: list[dict[str, str]],
    adapters: tuple[MCPResultAdapter, ...] | None = None,
) -> list[dict[str, Any]] | None:
    """Collect optional durable Surface blocks from authorized adapters.
    ...
    candidates = tuple(
        ...
        if callable(getattr(adapter, "durable_followup_content", None))
    )
    ...
    for adapter in candidates:
        try:
            method = adapter.durable_followup_content
            adapted = method(
                server_name=server_name,
                remote_name=remote_name,
                inline_images=[dict(item) for item in inline_images],
            ...
            _log.debug("MCP durable result adapter failed", exc_info=True)
    return blocks or None


def adapt_provider_messages(messages: list[Any]) -> list[Any]:
    """Apply active provider-boundary adapters to a request copy.

    Adapters run only inside their activation ContextVar. A failure leaves the
    original projection untouched so a tracing/sidecar helper cannot poison a
    normal model call.
    """
    adapted_messages = messages
    for adapter in _CURRENT.get():
        method = getattr(adapter, "adapt_provider_messages", None)
        if not callable(method):
            continue
        try:
            candidate = method(adapted_messages)
            if isinstance(candidate, list):
                adapted_messages = candidate
        except Exception:
            _log.debug("MCP provider message adapter failed", exc_info=True)
