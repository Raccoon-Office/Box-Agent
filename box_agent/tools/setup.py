...
def build_file_delivery_prompt() -> str:
    ...
    return (
        ...
        "有查看或复用价值的中间文件（清洗表、计算明细、初稿、支撑脚本）用 "
        "`publish_artifact(path, role=\"process\")` 登记；用户要求交付时对原文件改登记 "
        "role=\"deliverable\"，不要复制。输入原件仅被读取、缓存、缩略图和内部运行文件"
        "不调用 `publish_artifact`，也不归为过程。普通文字回答和纯操作任务不强造文件。"
        "修改已有文件也是独立交付结果；保存并验证可访问后登记，部分完成时先交付可用文件，"
        "明确未完成内容。PPT 默认交付 PPTX 和 HTML 两份，预览图不额外登记为正式产物。"
