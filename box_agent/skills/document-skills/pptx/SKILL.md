...
asset, `web` runs the trusted `search_free_image.py` once for all web rows, `ai`
uses generation directly, and `none` stays image-free. The initial web tier is
the zero-credential Openverse/Wikimedia chain. It accepts only metadata-relevant
CC0/Public Domain originals, records provenance, and converts a sourced row to
`use_existing`; it leaves `decision: generate` only after the free pool is
`exhausted` or `unavailable`. Only then call `generate_image` with `watermark:
false` for the remaining `generate` rows. Never skip free search for a web row or
replace a named real subject with fabricated documentary evidence; an AI fallback
is an explicitly labelled concept. Then run
...
| Resolve free web-image rows | `${BOX_AGENT_PYTHON:-python3} scripts/search_free_image.py assets/generated/manifest.json` (optional `--query 'SLIDE=concise entity terms'`; Openverse/Wikimedia only) |
