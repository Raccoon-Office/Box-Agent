...
像素证据有严格的新鲜度：修改某页 HTML 或 `base.css` 后，该页此前的 PNG 立即失效。必须先重渲再调用 Vision；不得反复查看旧 PNG、不得用“代码看起来正确”代替新像素。首次检查覆盖完整视觉、语义和设计要求，按页简记观察与问题。修复复验沿用原账本的页码、问题对象和成立条件，确认原问题及受影响区域的回归，不重做整页焦点、阅读路径、留白分析。相同 PNG 不因换问法而重查；意见矛盾时，只在局部图或必要 DOM / computed geometry 能提供新信息时补证。另一对对象的结论不能关闭原问题；证据不足就保留未决项，不继续试问，也不据此宣布 PASS。
...
讲稿验收不能只检查“字段存在”和“事实一致”。逐页抽读时必须确认它是站在讲者口吻下可直接朗读的完整串词：应推进解释、补充屏幕没有写出的因果、场景或过渡，不能把标题、标签、卡片名机械改写成枚举；不得包含独立的 Markdown 围栏行（如 ````` ``）、`plan/`、`research/`、`_trace/`、`grounded-knowledge.md`、编排器假设或生产备注。发现问题时，Box-Agent 静态新建先记入原问题账本，随 §4B 集中修正 `plan/slide_NN.md` 的 `## 口语讲稿` 与 `## 来源`，并入同次 `review-prep`，不在诊断期单独 build。其他路径保留修正计划后由 `deck.py build` 重新生成讲稿的步骤；所有路径都不能只手改派生的 `speech.md`。只有抽读覆盖全册、格式干净且讲稿不像屏显复述时，合同才可返回 `speech_aligned: yes`。
...
质量判断顺序是：**新鲜最终 PNG / Vision → DOM 与 computed geometry → 机检候选**。使用 `render.json`、PNG 和页面 DOM 判断问题，不读取或调试 `render.py`、`deck.py` 等工具实现来推翻质量门。技术硬阻塞包括：交付文件缺失/损坏、真实裁切或不可读溢出、浏览器运行错误、过期渲染、缺页或缺少 `present.html`；事实、语义和已规划视觉未兑现仍按 §4A 的质量要求处理。`cjkTypography`、`crowded`、`boxoverflow`、文字 bbox 相交、轻微低对比、细微换行/标点、没有改变事实含义的图表形式差异和审美偏好均为 advisory；除非新鲜像素或 DOM 证明其已造成真实不可读、遮挡、错义或用户明确要求被破坏，否则不能单独触发修改或 `blocked`。若报告与像素明显冲突，记录为 checker mismatch 并保留视觉更好的版本，不把 Review 变成渲染器调试任务。
...
4. 诊断阶段沿用 `_trace/review-issues.md` 这一份视觉问题账本，不覆盖制作期记录；每条包含页码、问题对象、成立条件、证据、严重度、根因、修法，以及是否影响事实、`base.css`、字体或讲稿。首轮覆盖简记每页结论；复验在原条目更新解决/未解决及受影响区域的回归结果，不重复输出整页分析或新建报告。即使没有问题，也要写明已覆盖全部页及 `remaining: none`，不能省略账本。任务含附件、使用 Research 或存在其他内容保真要求时，另行维护唯一事实核验报告 `_trace/content-fidelity.md`；附件内容硬伤从该报告合并进入同一修复批次，其他问题仍须由像素或渲染输出证实。停滞收口阶段只允许补齐或更新这两份正式产物，不得写页面或其他文件。
...
1. 按根因分组：先修能解决多页问题的 `base.css`，再修局部页。同页所有问题合并为一次修改；修改前读取计划与 HTML，并把当前 HTML 与最新 PNG 同时备份到 `_trace/review-backups/`。整个问题账本的“集中修改 → 准备新像素 → 复验”算 1 轮 refine，不按问题数、页数或执行者分别计数。
...
6. **仅其他环境及已有编辑：**局部修复确认后同步计划中的讲稿/来源，由父级运行一次 `deck.py review-prep "$DECK_DIR" --expected N`，再按 manifest 覆盖最终全册像素，不追加 contact/build。该命令仍只返回 `prepared / qa: not-run`。Box-Agent 静态新建已在步骤 3–4 完成正式待审和最终复验，不重复执行本步。
7. **共同出口：**无须修复时，Box-Agent 静态新建的 §4A 即为正式最终检查；有修复时使用步骤 3–4 的最终结果。其他路径使用步骤 6 的最终结果。将覆盖与结论写回 `_trace/review-issues.md`，必要时同步 `_trace/content-fidelity.md`，按 §5 写最终合同。最后看图后不再修改视觉源、render/prepare/build 或更换字体；任何视觉源变化后，按所属路径重新待审与验收。
...
每轮必须覆盖其规定范围，不能用固定总调用数跳过长册后续页面；这不授权重复检查同一未变化图或重置修复预算。图像载荷过大时按当前工具契约缩小批次；服务持续失败则保留未检查范围与真实错误，不能用 SVG/DOM 结构“应当正确”或“相信修复生效”为理由 `ready`。Box-Agent 的错误重试与降级边界以工具契约为准，未核验像素不能填写已通过。

Box-Agent 静态新建的讲稿修订并入同次正式待审，不另启动讲稿构建；其他路径只改讲稿不需要重渲。改变屏显内容必须回到字体准备和批量渲染步骤。
