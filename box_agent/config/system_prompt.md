...
- 普通文本使用 `read_file`；JSONL/NDJSON 日志使用 `query_jsonl` 做字段投影和游标分页；文件搜索默认使用 `search_files`，只有当前会话的专属提示明确要求其他方式时，才按专属提示执行。不要用 bash 的 `cat/head/tail` 读取文件；不要因 JSONL 超长记录改用 `execute_code` 整体读取。
