...
def observed_repair_after_one_failure(manager, *, scope="one-failure"):
    curator = manager.correction_curator
    subject = CorrectionSubject("tool", "export_probe", "v1")
    assert curator.observe_result(
        scope=scope, subject=subject, call_id="failed-once",
        arguments={"mode": "broken"}, success=False, error="unsupported layout",
    ) is None
    return curator.observe_result(
        scope=scope, subject=subject, call_id="validated-once",
        arguments={"mode": "fixed"}, success=True, content="validated output",
    )
...
def test_one_failure_then_matching_success_issues_evidence(manager):
    notice = observed_repair_after_one_failure(manager)
    assert notice is not None
    assert notice.verification_id.startswith("verification_")
    assert manager.list_corrections(include_inactive=True) == []
...
def test_evidence_does_not_cross_runs_replayed_calls_versions_or_failures(manager):
    ...
    curator.observe_result(scope="one", subject=subject, call_id="duplicate",
        arguments={"mode": "broken"}, success=False, error="unsupported layout")
    first_notice = curator.observe_result(scope="one", subject=subject, call_id="success",
        arguments={"mode": "fixed"}, success=True)
    assert first_notice is not None
    # The same failure call cannot be replayed to mint another receipt.
