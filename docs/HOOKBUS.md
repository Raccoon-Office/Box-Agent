...
八个调用。只接管正常 `stop` / `end_turn`，截断、掉流、参数超限及其他异常
终止继续走原有内核恢复路径。字符串参数保留全部空白；嵌套结构标签或重复
参数名按歧义块跳过；发现 invoke 嵌套时整次恢复放弃，不执行部分调用，
也不把参数内的一组 DSML 示例恢复成调用。
未闭合 invoke 可能连带跳过其后的完整块。正文已有内容或模型不匹配时保持
原响应；无法解析的块继续走原有
