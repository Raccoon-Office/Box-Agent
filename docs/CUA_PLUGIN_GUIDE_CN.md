...
这里的 `server_name` 必须匹配 `mcp.json` 的键名。如果 MCP server 使用的是 `computer-use`，就把两处都改成 `computer-use`。插件启动后会自动激活这个 server 已发现的工具，不需要模型先调用 `tool_search`；连接和重连仍由 Box-Agent 原有 MCP loader 管理。
...
3. 把规范化后的图片作为当前轮的 transient `user` 消息发送一次；它不进入 durable Surface，也不需要 provider hydrate。

SessionLog 和工具 trace 不保存图片 base64；SessionLog 中保留工具结果里的图片路径，sidecar 负责保存并校验图片 bytes。`~/.box-agent/log/sessions/` 下的文件是诊断 SessionTrace，主要记录 LLM/tool 请求；要检查工具结果路径，应查看对应的 `~/.box-agent/sessions/<session>/session.jsonl`。
