...
ACP 宿主可在 `initialize._meta.llm_binding` 传入默认模型绑定，格式与
`session/new._meta.llm_binding` 相同。启动记忆导入和维护等待握手后再启动，
使用该绑定；未提供此字段的独立宿主保留配置模型的兼容行为。

会话的显式绑定优先于握手默认值。标题、上下文压缩、追问、记忆和子 Agent
复用会话模型及 Auto 候选池：手动绑定保持模型不变，Auto 仅在宿主提供的池内
选模。宿主改变默认选择后，新的后台请求应携带最新的 `llm_binding`，不得靠
修改进程配置影响正在执行的会话。无会话的记忆整理也接受 `_meta.llm_binding`。
