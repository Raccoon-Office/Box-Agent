...
专家／专家团会话会把宿主 `packageSnapshot.skills` 中的目录加入当前会话的技能来源，标记为 `expert`。
该标记由加载器注册包目录时设置，不读取技能文件自报来源；没有包目录的旧专家继续使用技能原来源。
专家来源不读取普通技能的同名禁用名单；执行模式保持原规则。
专家团同时包含团长和成员各自的包目录，并保留每个目录所属专家的绑定名单，不向其他包传播绑定。包内同名技能以明确绑定的版本优先；同为绑定或同为未绑定时，继续按包目录先出现的优先。两个专家都绑定不同版本时，当前会话仍只选一个版本，尚不支持子任务按所属专家分别选取；
未绑定名称继续按原有来源查找。当前专家明确绑定的包内技能可覆盖 `allow_override: false` 的同名内置版本；普通会话和未绑定包技能仍遵守内置覆盖保护。
`roadmap` 等运行时保留名称不放行。已识别的绑定正文消失或损坏时保留不可用诊断，不静默回退到内置版本。
专家包技能不受 `disabledSkillNames` 影响，不修改或清除全局设置；内置、用户和连接器来源仍遵守原有禁用判断。
列表、推荐、正文读取和子任务委派继续遵守原有执行模式、任务范围及会话访问限制。
优先级只作用于会话加载器副本，普通会话及没有包目录的旧专家保持原规则。
包目录必须位于专家包内；技能正文的真实路径在发现和刷新时均不能越出该技能目录，防止符号链接绕过目录校验。
违规正文沿用已有的不可用技能诊断，只阻止该技能及依赖它的技能，其他技能继续可用；不读取包外正文。
刷新同时比较链接目标身份，文件大小和修改时间相同时也能识别目标变更；修复后可自动重新加载。

### 专家包 PPT 图片同步脚本

命令执行工具保留内置同步脚本路径，同时通过当前会话的 SkillLoader 动态取得明确绑定、实际选中的专家 `pptx` 的 `scripts/sync_image_manifest_status.js`。每次执行前刷新技能状态并确认资源真实路径在该技能目录及登记的专家目录内；未绑定、损坏、已删除或越界软链接不加入允许路径。普通会话不增加允许路径。原有命令格式、Node 调用方式和 manifest 路径检查保持不变，快速模式规则也不改变。
