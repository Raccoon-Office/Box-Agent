...
原生图片检查走第三条“仅本次请求”路径。规范化图片块只附加到主模型的一次请求，不进入持久消息历史；token 预算按解码后的图片尺寸估算，而不是按 Base64 字符长度计算。provider 发送前另按端点配置检查完整请求体字节数，详见[图片请求体大小控制](image-request-budget.md)。原始图片载荷不会写入 session trace 或 provider debug log。
...
压缩会创建新的历史 epoch。压缩结果中保留的 assistant 消息会清除旧的 provider usage，下一次 `prepare request` 从压缩后的消息面重新估算；loop 同时清除旧的 API token 锚点，直到新的 provider 响应提供新的 usage。
