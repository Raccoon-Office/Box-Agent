...
| Turn continuation judge | `turn_continuation.py`, `AgentLoopKernel`, `turn_continuation_judge`, `request_user_decision` | The auxiliary judge evaluates an isolated copy of the already-appended transcript, with only read-only runtime facts; it cannot mutate main history. Provider/client timeout remains authoritative, while caller cancellation or new user input interrupts the auxiliary request. | Hardens the existing model-guided continuation behavior without a public protocol or Session Log migration. | [PR #161](#2026-09-21--continuation-judge-transcript-isolation-and-timeout-authority-pr-161) |
...
| PPTX rendering runtime | `presentation-suite`, `render_runtime_windows.py`, `file_lock.py`, `ppt-platform.yml` | Windows owns each render with a fail-closed Job Object; macOS/Linux retain Unix process-group ownership; Doctor requires a real PNG render. | Adds native Windows runtime behavior; packaged OfficeV3 verification remains a separate boundary. | [2026-09-22 Windows rendering](#2026-09-22--pptx-windows-rendering-runtime-compatibility-pr-168) |
...
### 2026-09-22 — PPTX Windows rendering runtime compatibility (PR #168)

- **Change:** [PR #168, `fix(ppt): Integrate Windows rendering after R6d`](https://github.com/Raccoon-Office/Box-Agent/pull/168), merged as `db89ab2`.
- **Durable behavior:** Windows assigns each render worker to a private, non-inheritable, kill-on-close Job Object before the worker receives permission to start Playwright. Job creation or assignment failure fails the render rather than falling back to an unowned browser; cleanup waits for Job-owned descendants before releasing an admission slot. macOS and Linux retain the Unix-socket/process-group implementation.
- **Platform compatibility:** Windows uses a loopback TCP control connection with a random token and `msvcrt` file locks; other systems use their existing Unix socket and `flock` paths. Browser discovery honors the documented host overrides, file navigation uses `Path.as_uri()`, and the Bash installer accepts `BOX_AGENT_PYTHON` plus Windows virtual-environment interpreter paths.
- **Health check and CI:** Doctor now renders an offline minimal page and reports Chromium available only when the renderer exits successfully and produces a PNG. `.github/workflows/ppt-platform.yml` defines focused native Windows, macOS, and Linux render coverage, including browser tests; the workflow definition is not evidence of a later execution result.
- **Proof anchors:** `tests/test_presentation_platform.py`, `tests/test_presentation_windows_runtime.py`, `tests/test_presentation_platform_browser.py`, `tests/test_presentation_artifact_publication.py`, `tests/test_presentation_suite_bundle.py`, and `.github/workflows/ppt-platform.yml`.
- **Runtime and rollback:** Source tests and CI do not establish a packaged OfficeV3 runtime. Rebuild, install, restart, and run a fresh rendering probe before making that claim. Revert PR #168 as one change to restore the prior platform behavior; no data migration is introduced.

### 2026-09-21 — continuation judge transcript isolation and timeout authority (PR #161)

- **Change:** [PR #161, `fix(core): improve continuation judge context and timeout`](https://github.com/Raccoon-Office/Box-Agent/pull/161), merged as `751a3d5`.
- **Durable behavior:** The model-guided continuation judge forks the already-appended transcript, adds its internal request only to that copy, and receives read-only user-request and decision-tool facts; it does not mutate the main history or append the candidate twice.
- **Privacy and verdict boundary:** The projection removes reasoning, usage, request-only token accounting, and multimodal payloads. Only normal, text-only strict-JSON verdicts may continue the turn; malformed, tool-calling, or abnormal-finish responses do not.
- **Timeout and cancellation:** The auxiliary 15-second deadline is removed. Provider/client timeout and retry policy remain authoritative; caller cancellation or a newly queued user message cancels the isolated request and prevents its verdict from being accepted.
- **Compatibility:** Public ACP/CLI behavior, configuration, and Session Log schema are unchanged. This is internal kernel hardening of the bounded continuation policy, not a second durable transcript or user-decision protocol.
- **Proof anchors:** `tests/test_turn_continuation.py`, `tests/test_turn_continuation_thinking.py`, `tests/test_reasoning_disabled_policy.py`, `tests/test_llm_timeout.py`, `tests/test_model_profiles.py`, and `tests/test_context_compaction_e2e.py`.
- **Runtime and rollback:** PR source checks do not establish packaged-host or live-provider behavior. Rebuild, install, restart, and run a fresh task before making that claim. Revert PR #161 as one change to restore the former judge construction and deadline behavior; no data migration is required.
